...
var guitarRiffStartButton = null;
...
GuitarRiff.prototype.preload = function() {
  function bufferLoaded(audioBuffer) {
    ...
    guitarRiffStartButton.className = "btn";
    guitarRiffStartButton.addEventListener('click', function() {
      shakerapp.buttonClickHandler("guitar-riff", this);
    }, false);
  }

   createBufferLoader().load("sounds/guitar-riff.mp3", bufferLoaded.bind(this), audioContext);
};

function guitarRiffStartStop() {
  ...
  if (guitarRiff.running) {
    guitarRiffStartButton.className = "btn selected";
  } else {
    guitarRiffStartButton.className = "btn";
  }
}
